...
#define debug_printf(format, ...) { \
  char out_str[256]; \
  sprintf(out_str, format, ## __VA_ARGS__); \
  print_str(out_str); \
}
  ...
  /* Main and Proc_0 in the Ada version             */
{
  ...
  Next_Ptr_Glob = (Rec_Pointer) &Rec_Struct[0]; //alloca (sizeof (Rec_Type));
  Ptr_Glob = (Rec_Pointer) &Rec_Struct[1]; // alloca (sizeof (Rec_Type));
