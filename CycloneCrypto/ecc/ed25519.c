...
Ed25519State ed25519GenerateKeyPair_state;

_error_t ed25519GenerateKeyPair(const PrngAlgo *prngAlgo, void *prngContext,
   uint8_t *privateKey, uint8_t *publicKey)
{
   ...
   state = &ed25519GenerateKeyPair_state; // TODO: REPLACED MALLOC
 ...
 * @return Error code
 **/
Ed25519State ed25519GenerateSignature_state;

_error_t ed25519GenerateSignature(const uint8_t *privateKey,
   ...
   const void *context, uint8_t contextLen, uint8_t flag, uint8_t *signature)
{
   ...
   //Allocate working state
   state = &ed25519GenerateSignature_state;//cryptoAllocMem(sizeof(Ed25519State));
   //Failed to allocate memory?
   if(state == NULL)
      return ERROR_OUT_OF_MEMORY;
 ...
 * @return Error code
 **/
Ed25519State ed25519VerifySignature_state;

_error_t ed25519VerifySignature(const uint8_t *publicKey, const void *message,
   ...
   const uint8_t *signature)
{
   ...
   //Allocate working state
   state = &ed25519VerifySignature_state;//cryptoAllocMem(sizeof(Ed25519State));
   //Failed to allocate memory?
   if(state == NULL)
      return ERROR_OUT_OF_MEMORY;
