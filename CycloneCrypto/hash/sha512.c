 ...
 * @return Error code
 **/
Sha512Context sha512Compute_context;

_error_t sha512Compute(const void *data, size_t length, uint8_t *digest)
{
   //Allocate a memory buffer to hold the SHA-512 context
   Sha512Context *context = &sha512Compute_context;//(Sha512Context *)cryptoAllocMem(sizeof(sha512Compute));
