...
_error_t fsInit(void);

bool_t fsFileExists(const char_t *path);
_error_t fsGetFileSize(const char_t *path, uint32_t *size);
_error_t fsRenameFile(const char_t *oldPath, const char_t *newPath);
_error_t fsDeleteFile(const char_t *path);

FsFile *fsOpenFile(const char_t *path, uint_t mode);
_error_t fsSeekFile(FsFile *file, int_t offset, uint_t origin);
_error_t fsWriteFile(FsFile *file, void *data, size_t length);
_error_t fsReadFile(FsFile *file, void *data, size_t size, size_t *length);
void fsCloseFile(FsFile *file);

bool_t fsDirExists(const char_t *path);
_error_t fsCreateDir(const char_t *path);
_error_t fsRemoveDir(const char_t *path);

FsDir *fsOpenDir(const char_t *path);
_error_t fsReadDir(FsDir *dir, FsDirEntry *dirEntry);
