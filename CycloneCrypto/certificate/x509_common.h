...
_error_t x509ReadInt(const uint8_t *data, size_t length, uint_t *value);

_error_t x509ReadRsaPublicKey(const X509SubjectPublicKeyInfo *subjectPublicKeyInfo,
   RsaPublicKey *key);

_error_t x509ReadDsaPublicKey(const X509SubjectPublicKeyInfo *subjectPublicKeyInfo,
   DsaPublicKey *key);

bool_t x509IsSignAlgoSupported(X509SignatureAlgo signAlgo);
bool_t x509IsHashAlgoSupported(X509HashAlgo hashAlgo);

_error_t x509GetSignHashAlgo(const X509SignatureAlgoId *signAlgoId,
