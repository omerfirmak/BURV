...
_error_t x509ValidateCertificate(const X509CertificateInfo *certInfo,
   const X509CertificateInfo *issuerCertInfo, uint_t pathLength);

_error_t x509CheckSubjectName(const X509CertificateInfo *certInfo,
   const char_t *fqdn);

_error_t x509CheckNameConstraints(const char_t *subjectName,
